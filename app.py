...
# --- 4. SIDEBAR: TRIP & SENSOR INPUTS (ALL RESTORED) ---
logo_path = "logo.png"
...
# --- 5. SYSTEM ANALYSIS LOGIC (MEMORY SAFE) ---
st.divider()
if st.button("🚀 EXECUTE FULL SYSTEM ANALYSIS"):
    try:
        ...
        with st.status("Analyzing Electrical Patterns...") as s:
            ...
            df_elec = pd.DataFrame([[v_in, c_in, t_in]], columns=['Battery_Voltage', 'Battery_Current', 'Battery_Temperature'])
        ...
        if target_dist > range_p:
            st.error(f"❌ **Mission Impossible:** Destination is {target_dist}km away, but max range is {int(range_p)}km.")
        else:
        ...
        if health_p < 0.75:
            st.error(f"🔴 **URGENT:** Health critical ({health_p*100:.1f}%). Replacement mandatory.")
        elif health_p < 0.88:
...
with st.expander("📝 How to interpret the dashboard?"):
    st.write("- **SoC**: Battery energy. - **SoH**: Permanent health. - **Abnormal**: Energy waste detection.")